...
test('15. 血統切替UIの廃止後も馬名下の産駒情報を常時表示する', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  assert.ok(html.includes('${h.sire ? `<span class="horse-sire">'), '馬名欄に産駒情報の表示がない');
  assert.ok(html.includes('産駒</span>'), '種牡馬名が「○○産駒」の形式ではない');
  assert.ok(!html.includes("currentAxis === '血統' && h.sire"), '廃止した血統切替に表示が依存している');
});

test('16. 馬名と最新記事からnetkeibaへ移動できる', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  assert.ok(html.includes('function horseArticleLink(h)'), '最新記事リンクの生成処理がない');
  ...
  assert.ok(html.includes('${horseArticleLink(h)}'), '馬名欄に最新記事リンクが表示されていない');
  assert.ok(!html.includes('>戦績</a>'), '馬名リンクと重複する戦績ボタンが残っている');
});

test('17. 出馬表は3走前までを表示し4・5走前は計算用データとして保持する', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  const tableHead = html.match(/<table class="race-entry-table">[\s\S]*?<\/thead>/)?.[0] || '';
  assert.ok(tableHead.includes('<th>3走前</th>'), '3走前の列がない');
  assert.ok(!tableHead.includes('<th>4走前</th>') && !tableHead.includes('<th>5走前</th>'), '4・5走前が表示に残っている');
  assert.ok(html.includes('p4') && html.includes('p5'), '4・5走前の計算用データまで削除されている');
});

test('18. 枠・馬番を見失わず馬番順で表示する', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  assert.ok(html.includes('.race-entry-table th:nth-child(2), .race-entry-table td:nth-child(2) { position: sticky;'), '枠列が横スクロール時に固定されない');
  assert.ok(html.includes('const sorted = [...horses].sort((a,b) => a.num - b.num);'), '出馬表が馬番順になっていない');
  assert.ok(!html.includes('.race-entry-table th { padding: 6px 7px; position: sticky;'), '見出しの縦固定が馬の行に重なる');
});

test('19. 通算成績は同条件データがなくても同じ位置に表示する', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  assert.ok(html.includes('class="career-summary"'), '通算成績の固定レイアウトがない');
  assert.ok(html.includes("'同条件—'"), '同条件データなしの表示がない');
});

test('20. 馬ごとの個人印をレース・ユーザー単位で保存する', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  assert.ok(html.includes("const HORSE_MARK_OPTIONS = ['', '◎', '○', '▲', '△', '☆'];"), '個人印の選択肢がない');
  assert.ok(html.includes("const HORSE_MARK_LABELS = {'○': '◯'};"), '対抗印の表示サイズ調整がない');
  assert.ok(html.includes('kl:horse-marks:${user}:${race}'), '個人印がユーザー・レース単位で保存されない');
  assert.ok(html.includes('function setHorseMark(event, num, mark)'), '個人印の変更処理がない');
...
test('30. 馬の性齢と選択レースへの外部導線を表示する', () => {
  ...
  assert.ok(edge.includes('const age = parseInt(barei.match'), '性齢欄から年齢を取得していない');
  assert.ok(html.includes('function horseSexBadge('), '馬名横の性別表示がない');
  assert.ok(html.includes("const label = `${sex}${age || ''}`"), '牡4・牝3のように性別と年齢をまとめて表示していない');
