(function (root, factory) {
  ...
  function createPlan(horses, pace) {
    ...
    return (horses || []).map((horse, index) => {
      ...
      const frontStamina = Number.isFinite(Number(horse.frontStamina))
        ? clamp(Number(horse.frontStamina), 0, 1) : null;
      const closingPower = Number.isFinite(Number(horse.closingPower))
        ? clamp(Number(horse.closingPower), 0, 1) : null;
      const frontRole = style === '逃げ' ? 1 : style === '先行' ? 0.65 : 0;
      const closingRole = style === '追込' ? 1 : style === '差し' ? 0.8 : 0;
      // 脚質の平均では説明できない例外を、過去走から得た馬固有能力で補正する。
      // 強い逃げ馬は厳しい流れでも残り、強い差し馬はスローでも届き得る。
      const abilityBias =
        (frontStamina == null ? 0 : (frontStamina - 0.5) * 0.028 * frontRole) +
        (closingPower == null ? 0 : (closingPower - 0.5) * 0.028 * closingRole);
      return {
        ...horse,
        num: draw,
        style,
  ...
  function progressFor(entry, raceProgress) {
    ...
    const ability = (entry.abilityBias || 0) * Math.pow(latePhase, 1.8);
    const finish = entry.finishBias * Math.pow(p, 2.4);
    return clamp(p + early + pace + late + ability + finish, 0, 1.06);
  }
