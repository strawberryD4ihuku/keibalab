...
function normalizeTrackCondition(value) {
...
function summarizeWetTrack(runs, surface) {
  const sameSurface = (runs || []).slice(0, 20).filter(r =>
    (!surface || r.surface === surface) && finishScore(r) != null);
  const wetRuns = sameSurface.filter(r => {
    const condition = normalizeTrackCondition(r.trackCondition ?? r.baba);
    return condition && condition !== '良';
  });
  const dryRuns = sameSurface.filter(r =>
    normalizeTrackCondition(r.trackCondition ?? r.baba) === '良');
  const wetTrackFit = smoothedMean(wetRuns, () => true);
  const dryTrackFit = smoothedMean(dryRuns, () => true);
  return {
    wetTrackRuns: wetRuns.length,
    wetTrackTop3: wetRuns.filter(r => Number(r.rank) <= 3).length,
    wetTrackFit,
    wetTrackDelta: wetTrackFit != null && dryTrackFit != null
      ? clamp(wetTrackFit - dryTrackFit, -0.5, 0.5) : null,
  };
}
...
function summarizeConditions(runs, current) {
  ...
  return {
    ...
    courseStyleFit,
    ...summarizeWetTrack(recent, surface),
...
module.exports = {
  ...
  normalizeTrackCondition, summarizeWetTrack,
